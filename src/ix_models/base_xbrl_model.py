...
from src.constants.report_categories import ReportCategories
from src.exception import NotXbrlDirectoryException, NotXbrlTypeException
...
class BaseXbrlModel:
    ...
    def __init__(
        self, xbrl_zip_path: Union[str, Path], output_path: Union[str, Path]
    ) -> None:
        # XBRLファイルのzipファイルのパスを指定
        self.__xbrl_zip_path: Path = Path(xbrl_zip_path)
        self.__output_path: Path = Path(output_path)
        # XBRLファイルを解凍したディレクトリのパスを取得
        self.__directory_path: str = self.__unzip_xbrl()
        self.__xbrl_category: str = self.__xbrl_category()
        self.__head_item_key: str = str(
            Utils.string_to_uuid(Path(self.xbrl_zip_path).name)
        )

    @classmethod
    def xbrl_models(
        cls, xbrl_zip_dirs: Union[str, Path], output_path: Union[str, Path]
    ) -> Generator[Optional["BaseXbrlModel"], None, None]:
    ...
    def set_head_item_key(self, head_item_key: str) -> "BaseXbrlModel":
        self.__head_item_key = head_item_key
        return self

    def _set_manager(self) -> None:
    ...
    @property
    def xbrl_zip_path(self) -> str:
        return self.__xbrl_zip_path.as_posix()

    @xbrl_zip_path.setter
    def xbrl_zip_path(self, xbrl_zip_path: Union[str, Path]) -> None:
    ...
    @output_path.setter
    def output_path(self, output_path: Union[str, Path]) -> None:
    ...
    def __check_xbrl_files_in_dir(self, *keywords: str) -> bool:
        directory_path = Path(self.directory_path)
    ...
    def _xbrl_category_check(self, xbrl_category: str, *keywords: str) -> None:
