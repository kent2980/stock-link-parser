...
async def root():
    ...
    return {
        ...
        "endpoints": {
            "health": "/health",
            "head_item_key": "/api/v1/xbrl/head-item-key",
...
@app.get("/api/v1/xbrl/head-item-key")
async def get_head_item_key(zip_file_name: str = Query(..., description="zipファイル名")):
    """zipファイル名からhead_item_keyを取得

    Args:
        zip_file_name: zipファイル名（例: 081220250911556517.zip）

    Returns:
        head_item_key: zipファイル名から生成されたUUID
    """
    try:
        head_item_key = str(Utils.string_to_uuid(zip_file_name))
        return {
            "zip_file_name": zip_file_name,
            "head_item_key": head_item_key,
        }
    except Exception as e:
        logger.error(f"Failed to get head_item_key: {e}", exc_info=True)
